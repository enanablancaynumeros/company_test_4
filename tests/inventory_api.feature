Feature: Test the REST entrypoints of the inventory api
  ...
  Scenario: We can read the stock of a specific batch
    Given the following products in the system
      | name    |
      | chicken |
      | beans   |
  ...
  Scenario: We can read the history of a batch after modifying the stock multiple times
    Given the following products in the system
      | name    |
      | chicken |
      | beans   |
    And there is a batch with "3" items of "chicken" and expiry date for "next week"
    When the stock of the previously created stock is updated to "2"
    And the stock of the previously created stock is updated to "1"
    And the stock of the previously created stock is updated to "9"
    Then history of the previously created stock contains the following entries in the same order
      | stock:int |
      | 3         |
      | 2         |
      | 1         |
